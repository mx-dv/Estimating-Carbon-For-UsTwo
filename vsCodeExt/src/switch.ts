...
export function startProxy(context: vscode.ExtensionContext) {
...
export function deactivateProxy() {
    ...
    const config = vscode.workspace.getConfiguration('http');
    config.update('proxy', undefined, vscode.ConfigurationTarget.Global);
    config.update('proxyStrictSSL', undefined, vscode.ConfigurationTarget.Global);
}